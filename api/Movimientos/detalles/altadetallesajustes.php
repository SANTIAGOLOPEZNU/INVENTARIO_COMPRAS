...
try {
    if (!isset($params->DetailData->IdInsumosMat) || !isset($params->DetailData->Cantidad) || !isset($params->ajuste)) {
        echo json_encode(['resultado' => 'ERROR', 'mensaje' => 'Faltan datos requeridos']);
        exit;
    }
    // Asegúrate de que los nombres de los parámetros coincidan
    $stmt = $conn->prepare("INSERT INTO DetalleReciboRecepcion (IdInsumosMat, Cantidad, Id_Ajuste) VALUES (:IdInsumosMat, :Cantidad, :Id_Ajuste)");
    $stmt->bindParam(':IdInsumosMat', $params->DetailData->IdInsumosMat);
    $stmt->bindParam(':Cantidad',$params->DetailData->Cantidad);
    $stmt->bindParam(':Id_Ajuste', $params->ajuste);    
    if ($stmt->execute()) {
